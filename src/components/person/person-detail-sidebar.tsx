...
const inputClass =
  "flex h-9 w-full rounded-lg border border-amber-200 bg-white px-2.5 py-1.5 text-sm text-amber-950 shadow-xs transition-colors duration-150 placeholder:text-amber-600/40 focus:border-teal-400 focus:ring-2 focus:ring-teal-500/20 focus:outline-none";

const selectClass =
  "flex h-9 w-full rounded-lg border border-amber-200 bg-white px-2.5 py-1.5 text-sm text-amber-950 shadow-xs transition-colors duration-150 focus:border-teal-400 focus:ring-2 focus:ring-teal-500/20 focus:outline-none";
    ...
    <form onSubmit={handleSubmit} className="space-y-3">
      ...
      <div className="flex gap-2 pt-1">
        ...
        <button
          type="button"
          onClick={onCancel}
          className="rounded-lg border border-amber-200 px-3 py-2 text-xs font-medium text-amber-700 transition-colors duration-150 hover:bg-amber-50"
        >
          Cancelar
        </button>
      </div>
...
export function PersonDetailSidebar({
  ...
  return (
    <div
      ...
      className={`absolute top-0 right-0 z-40 h-full w-[340px] border-l border-amber-200 bg-white shadow-2xl transition-transform duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] ${
        isOpen && person ? "translate-x-0" : "translate-x-full"
      }`}
    >
      {person && (
        <div className="flex h-full flex-col overflow-y-auto">
          {/* ARIA: Parchment gradient header — from-amber-50 to-white creates
          ...
          <div className="bg-gradient-to-b from-amber-50 to-white px-5 pb-4 pt-4">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0 flex-1">
                {/* ARIA: Large serif name is the hero element — the person is the content */}
                <h2 className="font-serif text-xl font-semibold leading-tight text-amber-950">
                  {person.firstName}
                </h2>
              ...
              <button
                type="button"
                onClick={onClose}
                className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-amber-500 transition-colors duration-150 hover:bg-amber-100 hover:text-amber-800"
                aria-label="Cerrar"
              >
                <IconClose />
              </button>
          ...
          <div className="flex-1 space-y-5 p-5">
              ...
              <dl className="space-y-2.5 text-sm">
                {person.birthDate && (
                  <div className="flex gap-2">
                    <IconCalendar />
                    <div>
                      <dt className="text-xs text-amber-700">Nacimiento</dt>
                      ...
                      {person.birthPlace && (
                        <dd className="mt-0.5 flex items-center gap-1 text-xs text-amber-700">
                          <IconMapPin />
                          {person.birthPlace}
                        </dd>
                      )}
                ...
                {person.deathDate && (
                  <div className="flex gap-2">
                    <IconCalendar />
                    <div>
                      <dt className="text-xs text-amber-700">Fallecimiento</dt>
                      <dd className="text-amber-950">
                ...
                {person.maidenName && (
                  <div className="flex gap-2">
                    ...
                    <div>
                      ...
                      <dd className="text-amber-950">{person.maidenName}</dd>
                    </div>
                  </div>
                )}
                ...
                <ul className="space-y-2 text-sm">
                  {person.events.map((event, i) => (
                    <li key={i} className="flex gap-2">
                      ...
                      <div>
                        <span className="font-medium capitalize text-amber-950">
                          {event.type}
                        </span>
                        {event.date && (
                          <span className="ml-1 text-amber-700">
            ...
            {person.notes && (
              <section>
                ...
                <p className="rounded-lg bg-amber-50 px-3 py-2.5 text-sm leading-relaxed text-amber-900 italic">
                  {person.notes}
                </p>
              </section>
            )}
            ...
            <section>
              ...
              <div className="flex flex-col gap-2">
                <a
                  ...
                  className="flex w-full items-center justify-center gap-2 rounded-lg border border-amber-300 px-3 py-2 text-xs font-medium text-amber-800 transition-colors duration-150 hover:bg-amber-100"
                >
